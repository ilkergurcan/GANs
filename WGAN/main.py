...
if __name__ == "__main__":
    ...
    for epoch in range(train_steps):
        ...
        for batch in train:
            ...
            #fake_images = G(noise.float())
            fake_images = G.forward(noise.float())

            one = np.ones(batch_size)
            one = T.Tensor(one).to(device)
            one = T.reshape(one, (64, 1)).to(device)

            #D_output_real = D(batch.float())
            D_output_real = D.forward(batch.float())
            D_loss_real = T.mean(D_output_real)

            #D_output_fake = D(fake_images.float())
            D_output_fake = D.forward(fake_images.float())
            D_loss_fake = T.mean(D_output_fake)
            ...
            D_output = D.forward(fake_images.float())
            y = np.ones(batch_size)
            y = T.Tensor(y)
            y = T.reshape(y, (64, 1)).to(device)

            A_loss = -(T.mean(D_output))
            A_loss_total += A_loss
            A_loss.backward()
        ...
        #print(f"For epoch {epoch} Wasserstein Loss is : {Wasserstein_loss_total / len(train)}")
        print("///////////////////////////////////////////////////////////////////////////////")
        save_images()
